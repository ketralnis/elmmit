...
def api_route(path, method, doc, fields, db_call, returns):
    ...
    def fn():
        return 'hello, world!'
    fn.__name__ = route.basename

    app.route(path, methods=[method])(fn)
...
api_route('/api/get-comment', 'GET',
          'Retrieve a comment object by ID',
          {'comment_id': Field()},
          db.get_comment,
          returns=models.Comment)
...
LISTING_FIELDS = {
    'pager': Field('null if requesting the first page. otherwise the value you got for the previous page',
                   default=None),
    'limit': Field('how many to request. this is not limited by the server side, so be careful with large values',
                   type=int,
                   default=25),
}
...
api_route('/api/upvote-link', 'POST',
          'Upvote the given link',
          {'link_id': Field('the ID of the link to upvote'),
           'diff': Field('how many points to apply. this is not limited by the server',
                         type=int)},
          db.upvote_link,
          returns=models.Link)
